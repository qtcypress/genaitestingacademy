...
tripsage-rag/
  ...
  tests/                           blue_team.json, red_team.json (the designed cases)
...
Drop a `.md` file into `kb/` (optionally starting with a `last_updated: YYYY-MM-DD` line and
`## Section` headings), then toggle poison mode off/on or restart — it re-chunks and re-indexes,
and the new content is searchable and testable.
