...
class VectorStore:
    ...
    def build(self, chunks):
        ...
        toks_all = []
        for c in chunks:
            toks = tokenize(c["text"] + " " + c["section"])
...
class RAGEngine:
    ...
    def ask(self, query, tester="tester", top_k=None, threshold=None):
        ...
        # 4. detect indirect injection inside retrieved chunks
        used = []
        for h in hits:
            if h["score"] >= threshold and h["score"] >= best * 0.5:
                clean = h["text"]
                if INDIRECT.search(clean):
                    trace["flags"].append("indirect_injection_stripped:" + h["id"])
                    logger.info("GUARD   indirect injection detected in chunk %s -> ignored", h["id"])
                ...
                used.append(dict(h, clean=clean))
