...
function tab(t){document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x.dataset.t==t));
  ['ask','tests','kb','logs'].forEach(x=>document.getElementById(x).style.display=(x==t?'':'none'));
  if(t=='logs')loadLogs(); if(t=='tests')loadTests(); if(t=='kb')loadKB();}
async function status(){let r=await fetch('/api/status');let s=await r.json();
  document.getElementById('status').textContent=s.num_chunks+' chunks · '+s.kb_docs.length+' docs'+(s.poison?' · POISON ON':'');
  document.getElementById('poison').checked=s.poison;document.getElementById('topk').value=s.top_k;document.getElementById('thr').value=s.threshold;}
async function applyCfg(){let body={top_k:+document.getElementById('topk').value,threshold:+document.getElementById('thr').value,poison:document.getElementById('poison').checked};
    ...
    def do_GET(self):
        ...
        if u.path == "/api/status":
            return self._send(200, json.dumps({
                ...
                "poison": ENGINE.poison, "top_k": ENGINE.cfg.get("top_k", 4),
        ...
        if u.path == "/api/logs":
            ...
            fn = {"log": "tripsage.log", "trace": "trace.jsonl", "judge": "relevance_judgments.csv"}.get(kind, "tripsage.log")
